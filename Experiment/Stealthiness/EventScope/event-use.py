...
def simrank(G, r=0.8, max_iter=100, eps=1e-4):
    ...
    sim_prev = np.zeros((len(nodes), len(nodes)))
...
def main():
    ...
    with open("data.txt") as f:
        # Changed izip_longest to zip_longest for Python 3
        for l1,l2,l3,l4,l5 in itertools.zip_longest(*[f]*5):
        ...
        sorted_apps = []
        # Changed iteritems to items for Python 3
        for app_name, eventtypes in apps.items():
        ...
        print(f"Distance threshold for cluster split: {cluster_threshold}")
        print(f"Number of resulting clusters: {num_clusters}")
        print("")

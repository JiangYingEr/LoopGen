...
struct metadata {
    ...
    bit<1>  ipv4_forward_valid;
    macAddr_t ipv4_forward_dstAddr;
    egressSpec_t ipv4_forward_port;
}
...
control MyIngress(inout headers hdr,
    ...
    action ipv4_forward(macAddr_t dstAddr, egressSpec_t port) {
        meta.ipv4_forward_valid = 1;
        meta.ipv4_forward_dstAddr = dstAddr;
        meta.ipv4_forward_port = port;
    }
    ...
    apply {
        ...
        if (meta.ipv4_forward_valid == 1) {
            if (hdr.ipv4.ttl > 1) {
                hdr.ipv4.ttl = hdr.ipv4.ttl - 1;
                standard_metadata.egress_spec = meta.ipv4_forward_port;
                hdr.ethernet.srcAddr = hdr.ethernet.dstAddr;
                hdr.ethernet.dstAddr = meta.ipv4_forward_dstAddr;
            } else {
                mark_to_drop(standard_metadata);
            }
        }

...
# ======== CONFIGURATION ========
ROLE = "h3"      # change to "h2" / "h3" for other hosts
IFACE = f"{ROLE}-eth0"
PORT = 9999
...
shared_flag = {"stop": False, "fail": False, "round": 1}
...
def send_fail_notice():
    ...
    pkt_probe = Ether(src=SRC_MAC, dst="00:00:00:00:00:01") / IP(src=SRC_IP, dst="10.0.0.1") / UDP(dport=9999)
    sendp(pkt_probe, iface=IFACE, verbose=False)
